...
def parse_out(results):
    out = []
    total_length = 0  # Tieni traccia della lunghezza totale di tutti i testi aggiunti alla lista
    message = "DATO INTERROTTO, RIPETERE LA RICHIESTA PER GLI ARTICOLI NON ESTRATTI"
    message_length = len(message)

    for text in results:
        cleaned_text = re.sub(r'\n{2,}', '\n', text)  # Rimuovi le newline multiple
        current_length = len(cleaned_text)  # Calcola la lunghezza del testo corrente
        max_len = 100000 - message_length
        if total_length + current_length > max_len:
            # Se aggiungendo questo testo si supera il limite, aggiungi solo la parte che rientra nel limite
            remaining_space = max_len - total_length
            if remaining_space > 0:
                out.append(cleaned_text[:remaining_space])
            out.append(message)
            total_length += remaining_space  # Assicurati di aggiungere la lunghezza del testo effettivamente aggiunto
            print("Limite raggiunto, lunghezza totale dell'output:", total_length)
            break  # Interrompi il ciclo poiché abbiamo raggiunto il limite
        else:
            # Altrimenti, aggiungi tutto il testo alla lista
            out.append(cleaned_text)
            total_length += current_length  # Aggiorna la lunghezza totale

    print("Lunghezza totale finale dell'output:", total_length)  # Stampa la lunghezza totale per il debugging
    return out
